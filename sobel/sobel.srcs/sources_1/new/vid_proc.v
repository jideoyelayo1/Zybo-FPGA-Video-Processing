...
reg  [3:0] sw_On;
reg [7:0] gray,out,sobelX,sobelY;
reg [3:0] cnt = 0;
...
reg [11:0] XPtr; // X pointer
reg [1:0] wtPtr;//write pointer
reg [24:0] outMatrix [2:0];

// splits the 24-bit into RBG
...
always @ (posedge clk) begin    
    ...
    if(!n_rst) begin
        ...
        else if(cnt == 0) begin // Edge Detection - Sobel Filter Y
            out <= (outMatrix[0][23:16]*-1 + outMatrix[0][15:8]*-2 + outMatrix[0][7:0]*-1 +
                    outMatrix[1][23:16]* 0 + outMatrix[1][15:8]* 0 + outMatrix[1][7:0]* 0 +
                    outMatrix[2][23:16]* 1 + outMatrix[2][15:8]* 2 + outMatrix[2][7:0]* 1) ;end
        else if(cnt == 1) begin // Edge Detection - Sobel Filter X
            out <= (outMatrix[0][23:16]*-1 + outMatrix[0][15:8]* 0 + outMatrix[0][7:0]* 1 +
                    outMatrix[1][23:16]*-2 + outMatrix[1][15:8]* 0 + outMatrix[1][7:0]* 2 +
                    outMatrix[2][23:16]*-1 + outMatrix[2][15:8]* 0 + outMatrix[2][7:0]* 1) ;end
        else if(cnt == 2) begin // Edge Detection - Sobel Filter X
            sobelY <= (outMatrix[0][23:16]*-1 + outMatrix[0][15:8]*-2 + outMatrix[0][7:0]*-1 +
                        outMatrix[1][23:16]* 0 + outMatrix[1][15:8]* 0 + outMatrix[1][7:0]* 0 +
                        outMatrix[2][23:16]* 1 + outMatrix[2][15:8]* 2 + outMatrix[2][7:0]* 1) ;
            sobelX <= (outMatrix[0][23:16]*-1 + outMatrix[0][15:8]* 0 + outMatrix[0][7:0]* 1 +
                        outMatrix[1][23:16]*-2 + outMatrix[1][15:8]* 0 + outMatrix[1][7:0]* 2 +
                        outMatrix[2][23:16]*-1 + outMatrix[2][15:8]* 0 + outMatrix[2][7:0]* 1) ;
            
            sobelProd <= sobelX*sobelX + sobelY*sobelY;
            out <= sobelRes;
        end
